...
export function AdminDashboard() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Number(formData.capacidad) < 1) {
      setMessage({ type: 'error', text: 'Error de capacidad.' });
      setLoading(false);
      return;
    }
                ...
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition ${activa ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'
                        }`}
                    >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {DAYS_OF_WEEK.map(day => (
                        <button
                          ...
                          className={`px-3 py-1.5 text-sm rounded-md border transition ${selectedDays.includes(day)
                ...
                {fetching ? (
                  ...
                  routesList.map(route => (
                    <div
                      ...
                      className={`p-3 rounded-lg border cursor-pointer transition relative group ${editingId === route.id
